...
class ZoneManager:
    ...
    def resolve_direction(self, state: ZoneState) -> Tuple[int, str]:
        ...
        entry_pt = state.entry_point
        vx = exit_pt[0] - entry_pt[0]
        vy = exit_pt[1] - entry_pt[1]
        if fx == 0 and fy == 0:
            return 0, ''
        dot = fx * vx + fy * vy
        mag_flow = max((fx ** 2 + fy ** 2) ** 0.5, 1e-6)
        mag_vec = max((vx ** 2 + vy ** 2) ** 0.5, 1e-6)
        cos_theta = dot / (mag_flow * mag_vec)
        if cos_theta >= 0:
            return 5, '正向前出'
        return 7, '反向前出'
